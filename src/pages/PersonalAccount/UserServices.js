    ...
    useEffect(() => {
        currentUser &&
            userInfo(currentUser?.id).then((res) =>
                setUserService({ isLoading: true, data: res.services })
            );
        window.scrollTo(0, 0);
    }, [currentUser?.id]);
    ...
    return (
        <div className="px-2 px-sm-4 px-xxl-5 pb-4 pb-xxl-5">
            ...
            {userService?.data?.length > 0 && (
                <div className="mb-4">
                    <NavLink to="create" className="btn btn-1 fs-12 mx-auto mt-4 mt-xl-5">
                        Создать услугу
                    </NavLink>
                </div>
            )}
            {userService.isLoading ? (
                userService?.data?.length !== 0 ? (
                    ...
                    <div>
                        <img
                            src="/img/services-gray.svg"
                            alt="услуги"
                            className="img-fluid d-block mx-auto mb-4 mb-xl-5"
                        />
                        <div className="fs-12 text-center">
                            Предоставляете услуги риелтора, дизайнера, ремонтного рабочего
                            или грузоперевозчика? <br /> Создайте объявление об услугах и
                            находите заказчиков.
                        </div>
                        <Link
                            to="create"
                            className="btn btn-1 fs-12 mx-auto mt-4 mt-xl-5"
                        >
                            Создать
                        </Link>
                    </div>
